 ...
 * @class QueryManager
 * @constructor
 * @param {Array} [initSourcesList] valikuline allikate nimekiri
 */
var QueryManager = function(initSourcesList) {
  var selectedSourcesList = initSourcesList || sources.getAllSourcesList();
  ...
  function query(queryString, queryParams, sourcesList) {
    ...
    for (i=0; i<sourcesList.length; i+=1) {
      ...
      promise = sources.getSource(sourceName).query(queryString, queryParams);
