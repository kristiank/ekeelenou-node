...
var loadSources = ['vikiMediator', 'thesMediator', 'qsMediator',
                   'ekssMediator', 'vakkMediator', 'ekkrMediator',
                   'iesMediator', 'evsMediator', 'knabeeMediator',
                   'knabmmMediator', 'synMediator', 'amssMediator',
                   'etyMediator'];
for (var i=0; i<loadSources.length; i+=1) {
  var mediator = require('./sources/'+loadSources[i]);
  sourceManager.register(mediator);
}
...
exports.initUserSocket = function(io, socket) {
  ...
  // @todo: what should we do on reconnect?
  socket.on('reconnect', function(message) {
    console.log('reconnect on socket id: ', socket.id);
    // @todo: 
  });
  ...
  socket.emit('availableSources',
              JSON.stringify(sourceManager.getAllSourcesList()));
  ...
  socket.on('query', function(JSONmessage) {
    ...
    for(i=0; i<promises.length; i+=1) {
      ...
      promise.then(function(message){
        console.log('socket queryData: ', message);
        socket.emit('queryData', JSON.stringify(message));
